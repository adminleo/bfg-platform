...
class TokenStatus(str, enum.Enum):
    EMITTED = "emitted"       # Created by system owner
    SOLD = "sold"             # Purchased by user or expert
    ASSIGNED = "assigned"     # Bound to a specific UID
    ACTIVATED = "activated"   # User clicked link, diagnostic started
    CONSUMED = "consumed"     # Diagnostic completed, result stored
    EXPIRED = "expired"       # Token expired without use
...
class DiagnosticToken(Base):
    ...
    diagnostic_type: Mapped[str] = mapped_column(String(50))  # scil, big_five, values, eq, resilience, etc.
    ...
    # Reseller tracking
    sold_by_expert_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("experts.id"), nullable=True)
    ...
    sold_by_expert: Mapped["Expert | None"] = relationship(back_populates="sold_tokens")
