...
async def import_result(data: ImportResultRequest, user_id: uuid.UUID | None = None, db: AsyncSession = Depends(get_db)):
...
async def list_imported_results(user_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def delete_imported_result(result_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
